...
Основной режим работы — Ultralytics YOLOv8 с весами, обученными на датасете
DeepPCB (ТЗ п. 4.1.2 и 4.5.1). Если файла с весами нет или пакет ``ultralytics``
не установлен, используется fallback-детектор — эвристика на OpenCV. Fallback
предназначен исключительно для демонстрационных целей (сквозной прогон системы
без ML-весов) и не удовлетворяет количественным требованиям ТЗ п. 4.1.3.
"""
